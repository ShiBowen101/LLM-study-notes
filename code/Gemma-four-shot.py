...
for i in range(len(data_api)-4):
    messages = [
        ...
        {"role": "user", "content": data_api[-1]},
        {"role": "model", "content": labels[-1]},
        {"role": "user", "content": data_api[-2]},
        {"role": "model", "content": labels[-2]},
        {"role": "user", "content": data_api[-3]},
        {"role": "model", "content": labels[-3]},
        {"role": "user", "content": data_api[-4]},
        {"role": "model", "content": labels[-4]},
...
accuracy = 0.0
accuracy = count/(len(labels)-4)
print(f"two-shot accuracy:{accuracy}")
df = pd.DataFrame(data=answers, columns=["prediction(four-shot)"])
df.to_csv("prediction_four-shot_Gemma.csv")