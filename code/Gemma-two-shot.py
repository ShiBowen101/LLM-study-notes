...
for i in range(len(data_api)-2):
    ...
    if last_response == str(labels[i]):
        count = count + 1
    print(f"{i}/{len(labels)-2}")
accuracy = 0.0
accuracy = count/(len(labels)-2)
print(f"two-shot accuracy:{accuracy}")
df = pd.DataFrame(data=answers, columns=["prediction(two-shot)"])
# df.to_csv("prediction_two-shot_Gemma.csv")